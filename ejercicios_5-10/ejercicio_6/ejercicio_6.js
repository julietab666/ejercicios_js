...
const pacientes_mayus = pacientes.map(paciente => {
    const nombre_comp = `${paciente.nombre} ${paciente.apellido}`
    return nombre_comp.toUpperCase();
});

const a_disponible = ambulancias.filter(ambulancia => ambulancia.estado === 'disponible');

console.log(a_disponible);

const a_disponible_km = ambulancias.filter(ambulancia => ambulancia.estado === 'disponible' && ambulancia.kmRecorridos > 5000);

console.log(a_disponible_km);

const p_activos = pacientes.filter(paciente => paciente.activo === true);

console.log(p_activos);